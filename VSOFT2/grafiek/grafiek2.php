...
$stmt = $con->prepare("SELECT id, klantNummer, gasVerbruik, energieVerbruik, jaar, maand FROM data where klantNummer = 2 AND jaar = 2021");
$stmt->bind_param("s", $id[2]);
$stmt->execute();
...
echo "<script>energiedata3 = " .$energieverbruik. "</script>";
  ...
  $stmt = $con->prepare("SELECT id, klantNummer, gasVerbruik, energieVerbruik, jaar, maand FROM data where klantNummer = 2 AND jaar = 2020");
  $stmt->bind_param("s", $id[2]);
  $stmt->execute();
  $result = $stmt->get_result();
...
$stmt = $con->prepare("SELECT id, klantNummer, gasVerbruik, energieVerbruik, jaar, maand FROM data where klantNummer = 2 AND jaar = 2019");
$stmt->bind_param("s", $id[2]);
$stmt->execute();
$result = $stmt->get_result();
        ...
        <div class="container-fluid">
          ...
          <div class="collapse navbar-collapse" id="navbarSupportedContent">
            <ul class="navbar-nav me-auto mb-2 mb-lg-0">
              ...
              <li class="nav-item">
                <a class="nav-link active" aria-current="page" href="vergelijk2.php">Grafiek Statistiek</a>
              </li>
            <li class="nav-item">
